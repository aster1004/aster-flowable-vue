 ...
 * @Date: 2024-02-29 14:45:00
 * @FilePath: \aster-flowable-vue\src\views\generator\baseclass\index.vue
 * @Description: 基类管理
  ...
  <div class="table-box">
      ...
      <el-form ref="queryForm" :model="queryParams" :inline="false" @keyup.enter="handleQuery()">
        <div class="grid-box">
          <div class="grid-column">
            <el-form-item :label="$t('label.generator.baseCode')" prop="code">
              <el-input
                v-model="queryParams.code"
                :placeholder="$t('placeholder.generator.baseCode')"
                clearable
              />
            </el-form-item>
          </div>
    ...
    <div class="card table-main">
      ...
      >
        ...
        <el-table-column
          prop="code"
          :label="$t('label.generator.baseCode')"
          header-align="center"
          align="center"
        />
        <el-table-column
          prop="fields"
          :label="$t('label.generator.baseField')"
          header-align="center"
          align="center"
        />
        <el-table-column
...
<script setup lang="ts">
  import { baseClassPageApi, baseClassDeleteApi } from '@/api/gen/index';
  ...
  /** 查询条件 */
  const queryParams = reactive<Generator.BaseClassParams>({
    code: '',
    pageNum: 1,
    pageSize: 10,
  });
  /** 数据列表 */
  const dataList = ref<Generator.BaseClassInfo[]>();
  /** 总数 */
  const total = ref<number>(0);
  /** 已选择列表 */
  const selectedList = ref<Generator.BaseClassInfo[]>();
  ...
  const handleQuery = () => {
    loading.value = true;
    baseClassPageApi(queryParams).then(({ data }) => {
  ...
  const handleDelete = (key?: string) => {
    ...
    ElMessageBox.confirm(t('delete.confirm'), t('common.tips'), {
      ...
      .then(() => {
        baseClassDeleteApi(val).then((res) => {
