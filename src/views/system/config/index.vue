 ...
 * @Date: 2024-01-30 10:47:51
 * @FilePath: \aster-flowable-vue\src\views\system\config\index.vue
 * @Description: 系统配置
      ...
      <el-form ref="queryForm" :model="queryParams" :inline="false" @keyup.enter="handleQuery()">
        <div class="grid-box">
          <div class="grid-column">
            <el-form-item :label="$t('label.config.configName')" prop="configName">
              <el-input
          ...
          <div class="grid-column">
            <el-form-item :label="$t('label.config.configKey')" prop="configKey">
              <el-input
          ...
          <div class="grid-column" v-show="!searchCollapsed">
            <el-form-item :label="$t('label.config.configType')" prop="configType">
              <dict-select
                v-model="queryParams.configType"
                dict-type="whether"
                clearable
                :placeholder="$t('placeholder.config.configType')"
              />
            </el-form-item>
          </div>
    ...
    <div class="card table-main">
      ...
      <div class="table-header">
        <div class="header-button-lf">
          <el-button type="primary" v-hasPerm="['sys:config:add']" @click="handleAdd">
            <i class="iconfont icon-xinzeng pr-5px"></i>{{ $t('button.add') }}
          </el-button>
          <el-button type="danger" v-hasPerm="['sys:config:delete']" @click="handleDelete()">
      ...
      >
        ...
        <el-table-column
          prop="configName"
          :label="$t('label.config.configName')"
          header-align="center"
          align="center"
        />
        <el-table-column
          prop="configKey"
          :label="$t('label.config.configKey')"
          header-align="center"
          align="center"
        />
        <el-table-column
          prop="configValue"
          :label="$t('label.config.configValue')"
          header-align="center"
          align="center"
        />
        <el-table-column
          prop="configType"
          :label="$t('label.config.configType')"
          header-align="center"
          align="center"
        >
          <template #default="scope">
            <dict-tag dict-type="whether" :value="scope.row.configType" />
          </template>
        </el-table-column>
        <el-table-column
          prop="status"
...
<script setup lang="ts">
  import { configDeleteApi, configPageApi } from '@/api/sys/config';
  ...
  /** 数据列表 */
  const dataList = ref<Config.ConfigInfo[]>();
  /** 总数 */
  const total = ref<number>(0);
  /** 已选择列表 */
  const selectedList = ref<Config.ConfigInfo[]>();
  /** 查询条件 */
  const queryParams = reactive<Config.ConfigParams>({
    configKey: '',
    configName: '',
    configType: '',
    pageNum: 1,
  ...
  const handleQuery = () => {
    loading.value = true;
    configPageApi(queryParams).then(({ data }) => {
   ...
   * @return {*}
   */
  const handleSelectionChange = (val: Config.ConfigInfo[]) => {
