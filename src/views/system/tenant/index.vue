      ...
      <el-form ref="queryForm" :model="queryParams" :inline="false" @keyup.enter="handleQuery()">
        <div class="grid-box">
          <div class="grid-column">
            <el-form-item :label="$t('label.tenant.contactUserName')" prop="contactUserName">
              <el-input
          ...
          <div class="grid-column">
            <el-form-item :label="$t('label.tenant.companyName')" prop="companyName">
              <el-input
    ...
    <div class="card table-main">
      ...
      <div class="table-header">
        <div class="header-button-lf">
          <el-button type="primary" v-hasPerm="['sys:tenant:add']" @click="handleAdd">
            <i class="iconfont icon-xinzeng pr-5px"></i>{{ $t('button.add') }}
          </el-button>
          <el-button type="danger" v-hasPerm="['sys:tenant:delete']" @click="handleDelete()">
      ...
      >
        ...
        <el-table-column
          ...
          :label="$t('label.tenant.contactUserName')"
          header-align="center"
          align="center"
        />
        <el-table-column
          prop="contactPhone"
          :label="$t('label.tenant.contactPhone')"
          header-align="center"
          align="center"
        />
        <el-table-column
          prop="companyName"
          :label="$t('label.tenant.companyName')"
          header-align="center"
          align="center"
        />
        <el-table-column
          prop="licenseNumber"
          :label="$t('label.tenant.licenseNumber')"
          header-align="center"
          align="center"
        />
        <el-table-column
...
<script setup lang="ts">
  import { tenantPageApi, tenantDeleteApi } from '@/api/sys/tenant';
  ...
  /** 查询条件 */
  const queryParams = reactive<Tenant.TenantParams>({
    contactUserName: '',
    companyName: '',
    status: '',
    pageNum: 1,
  ...
  /** 数据列表 */
  const dataList = ref<Tenant.TenantInfo[]>();
  /** 总数 */
  const total = ref<number>(0);
  /** 已选择列表 */
  const selectedList = ref<Tenant.TenantInfo[]>();
  ...
  const handleQuery = () => {
    loading.value = true;
    tenantPageApi(queryParams).then(({ data }) => {
   ...
   * @return {*}
   */
  const handleSelectionChange = (val: Tenant.TenantInfo[]) => {
