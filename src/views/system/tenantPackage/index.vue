      ...
      <el-form ref="queryForm" :model="queryParams" :inline="false" @keyup.enter="handleQuery()">
        <div class="grid-box">
          <div class="grid-column">
            <el-form-item :label="$t('label.tenant.packageName')" prop="packageName">
              <el-input
    ...
    <div class="card table-main">
      ...
      <div class="table-header">
        <div class="header-button-lf">
          <el-button type="primary" v-hasPerm="['sys:tenantPackage:add']" @click="handleAdd">
            <i class="iconfont icon-xinzeng pr-5px"></i>{{ $t('button.add') }}
          </el-button>
          <el-button type="danger" v-hasPerm="['sys:tenantPackage:delete']" @click="handleDelete()">
      ...
      >
        ...
        <el-table-column
          prop="packageName"
          :label="$t('label.tenant.packageName')"
          header-align="center"
          align="center"
        />
        <el-table-column
        ...
        <el-table-column :label="$t('label.operate')" align="center" class-name="operation">
          <template #default="scope">
            <el-button
              ...
              v-hasPerm="['sys:tenantPackage:edit']"
            >
            ...
            <el-button
              ...
              v-hasPerm="['sys:tenantPackage:delete']"
            >
...
<script setup lang="ts">
  import { tenantPackagePageApi, tenantPackageDeleteApi } from '@/api/sys/tenantPackage';
  ...
  /** 查询条件 */
  const queryParams = reactive<TenantPackage.TenantPackageParams>({
    packageName: '',
    status: '',
    pageNum: 1,
    pageSize: 10,
  ...
  /** 数据列表 */
  const dataList = ref<TenantPackage.TenantPackageInfo[]>();
  /** 总数 */
  const total = ref<number>(0);
  /** 已选择列表 */
  const selectedList = ref<TenantPackage.TenantPackageInfo[]>();
  ...
  const handleQuery = () => {
    loading.value = true;
    tenantPackagePageApi(queryParams).then(({ data }) => {
   ...
   * @return {*}
   */
  const handleSelectionChange = (val: TenantPackage.TenantPackageInfo[]) => {
  ...
  const handleDelete = (key?: string) => {
    ...
    ElMessageBox.confirm(t('delete.confirm'), t('common.tips'), {
      ...
      .then(() => {
        tenantPackageDeleteApi(val).then((res) => {
