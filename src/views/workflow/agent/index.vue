    ...
    <div class="card table-main">
      ...
      <div class="table-header">
        <div class="header-button-lf">
          <el-button type="primary" v-hasPerm="['wf:agent:add']" @click="handleAdd">
            <i class="iconfont icon-xinzeng pr-5px"></i>{{ $t('button.add') }}
          </el-button>
          <el-button type="danger" v-hasPerm="['wf:agent:delete']" @click="handleDelete()">
            <i class="iconfont icon-shanchu pr-5px"></i>{{ $t('button.delete') }}
          </el-button>
          <el-button type="danger" v-hasPerm="['wf:agentLog:list']" @click="handleOpenLog()">
      ...
      >
        <el-table-column
          ...
          align="center"
          width="50"
          :selectable="(row) => userStore.isSuperAdmin || row.delegatorId === userStore.userInfo.id"
        />
        <el-table-column
          prop="delegatorName"
        ...
        <el-table-column
          prop="startTime"
          :label="$t('workflow.label.agentStart')"
          header-align="center"
          align="center"
        />
        <el-table-column
          prop="endTime"
          :label="$t('workflow.label.agentEnd')"
          header-align="center"
          align="center"
        />
        <el-table-column
          prop="agentType"
          :label="$t('workflow.label.agentType')"
          header-align="center"
          align="center"
        >
          <template #default="scope">
            <dict-tag dict-type="agent_type" :value="scope.row.agentType" />
          </template>
        </el-table-column>
        <el-table-column
          prop="status"
        ...
        <el-table-column :label="$t('label.operate')" align="center" class-name="operation">
          <template #default="scope">
            <el-button
              ...
              v-hasPerm="['wf:agent:edit']"
              v-if="userStore.isSuperAdmin || scope.row.delegatorId === userStore.userInfo.id"
            >
...
<script setup lang="ts">
  import { agentPageApi, agentDeleteApi } from '@/api/workflow/agent';
  ...
  /** 查询条件 */
  const queryParams = reactive<Agent.AgentParams>({
    name: '',
    startTime: '',
    endTime: '',
    status: '',
  ...
  /** 数据列表 */
  const dataList = ref<Agent.AgentInfo[]>();
  /** 总数 */
  const total = ref<number>(0);
  /** 已选择列表 */
  const selectedList = ref<Agent.AgentInfo[]>();
  /** 日期时间 */
  const dateValue = ref('');
  ...
  const handleQuery = () => {
    loading.value = true;
    agentPageApi(queryParams).then(({ data }) => {
   ...
   * @return {*}
   */
  const handleSelectionChange = (val: Agent.AgentInfo[]) => {
   ...
   */
  const handleOpenLog = () => {
    router.push({ path: '/workflow/agentLog' });
  };
</script>
