    ...
    <div class="card table-main">
      ...
      <div class="table-header">
        <div class="header-button-lf">
          <el-button type="primary" v-hasPerm="['wf:app:add']" @click="handleAdd">
            <i class="iconfont icon-xinzeng pr-5px"></i>{{ $t('button.add') }}
          </el-button>
          <el-button type="danger" v-hasPerm="['wf:app:delete']" @click="handleDelete()">
...
<script setup lang="ts">
  import { appPageApi, appDeleteApi } from '@/api/workflow/app';
  ...
  /** 查询条件 */
  const queryParams = reactive<WorkApp.AppParams>({
    name: '',
    status: '',
    pageNum: 1,
    pageSize: 10,
  ...
  /** 数据列表 */
  const dataList = ref<WorkApp.AppInfo[]>();
  /** 总数 */
  const total = ref<number>(0);
  /** 已选择列表 */
  const selectedList = ref<WorkApp.AppInfo[]>();
   ...
   * @return {*}
   */
  const handleSelectionChange = (val: WorkApp.AppInfo[]) => {
