  ...
  function raw(): Token | void | null {
    ...
    while (i < input.length) {
      ...
      if (state === rawStates.ESCAPE) {
        if (Object.prototype.hasOwnProperty.call(escapes, ch) || ch === '$') {
          buffer += ch;
          i++;
  ...
  function char() {
    ...
    if (ch === '\\') {
      ...
      if (
        ...
        Object.prototype.hasOwnProperty.call(escapes, nextCh)
      ) {
        i++;
        ch =
          nextCh === 'b'
  ...
  function template(): Token | void | null {
    ...
    while (i < input.length) {
      ...
      if (state === stringStates.ESCAPE) {
        if (Object.prototype.hasOwnProperty.call(escapes, ch) || ch === '`' || ch === '$') {
          i++;
  ...
  function stringLiteral() {
    ...
    while (i < input.length) {
      ...
      if (state === stringStates.START) {
        ...
        if (Object.prototype.hasOwnProperty.call(escapes, ch) || ch === startQuote) {
          i++;
          state = stringStates.START_QUOTE_OR_CHAR;
        } else {
          const pos = position(input.substring(index, i + 1));
