...
export async function runSequence<T, U>(arr: Array<T>, fn: (item: T, index: number) => Promise<U>) {
...
export class AsyncEvaluator extends (Evaluator as any) {
  ...
  async document(ast: { type: 'document'; body: Array<any> }) {
    ...
    const content = await runSequence(ast.body, async (item) => {
      const result = this.evalute(item);
  ...
  async unary(ast: { op: '+' | '-' | '~' | '!'; value: any }) {
    const value = await this.evalute(ast.value);
  ...
  async object(ast: { members: Array<{ key: string; value: any }> }) {
    const object: any = {};
    await ast.members.reduce(async (promise: any, { key, value }: any, index: number) => {
      await promise;
  ...
  async conditional(ast: { type: 'conditional'; test: any; consequent: any; alternate: any }) {
  ...
  async funcCall(this: any, ast: { identifier: string; args: Array<any> }) {
    ...
    const fn =
      ...
      this[fnName] ||
      (Object.prototype.hasOwnProperty.call(this.filters, ast.identifier) &&
   ...
   */
  async fnIF(condition: () => any, trueValue: () => any, falseValue: () => any) {
  ...
  async fnXOR(...condtions: Array<() => any>) {
    ...
    return !!((await runSequence(condtions, (c) => c())).filter((item) => item).length % 2);
  }
  ...
  fnARRAYMAP(value: any, iterator: any) {
    ...
    return (Array.isArray(value) ? value : []).reduce(async (promise, item, index) => {
  ...
  async fnARRAYFILTER(value: any, iterator: any) {
    ...
    return await (Array.isArray(value) ? value : []).reduce(async (promise, item, index) => {
      const arr = await promise;
  ...
  async fnARRAYFINDINDEX(arr: any[], iterator: any) {
    ...
    await (Array.isArray(arr) ? arr : []).reduce(async (promise: any, item: any, index: number) => {
      await promise;
      const hit = await this.callAnonymousFunction(iterator, [item, index]);
  ...
  async fnARRAYSOME(arr: any[], iterator: any) {
    ...
    const result = await (Array.isArray(arr) ? arr : []).reduce(
